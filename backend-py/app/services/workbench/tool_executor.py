...
async def executeToolBatch(
    toolUses: list[object], executeOne: Callable[[object], object], options: dict[str, object] | None = None
) -> list[object]:
    ...
    canRunInParallel = cast('Callable[[object], object] | None', opts.get('can_run_in_parallel'))
    onResult = cast('Callable[[object], object] | None', opts.get('on_result'))
    if parallel and canRunInParallel:
        allSafe = all((canRunInParallel(tu) for tu in toolUses))
        if allSafe:
            tasks = [cast(Awaitable[object], executeOne(tu)) for tu in toolUses]
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    final.append(
                        {
                            'tool_call_id': getattr(toolUses[i], 'get', lambda k, d=None: d)('id', ''),
                            'role': 'tool',
    ...
    results = []
    for tu in toolUses:
        result = await cast(Awaitable[object], executeOne(tu))
