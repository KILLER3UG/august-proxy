...
def register() -> None:
    ...
    tool_registry.register(
        'circuit_detect_ic',
        'Identify which real part a schematic TOPOLOGY corresponds to — '
        'deterministic, offline, explainable subgraph matching over net '
        'connectivity (kinds, shared nets, ground, net degree) against a '
        'library of known topologies: RC low/high-pass, snubber, capacitive '
        'dropper, divider, current-sense shunt, LED/diode + series R, '
        'common-emitter/collector, differential pair, current mirror, '
        'push-pull pairs, LC pi/T/tank, reactive feedback loop, debounce RC, '
        'switch pull-up/pull-down, '
        'amplifier instances. Per match: confidence, the matched subgraph '
        '(refs + nets), evidence, limits (what the graph cannot prove) and '
        'part-number candidates with estimated flags; plus "unexplained" refs. '
        'Read confidence as a scale and never overstate it: at/above '
        f'{ic_detect.MIN_DEFINITE_CONFIDENCE} it may be stated as an '
        'identification; 0.7-0.79 the shape matched and the ROLE is the '
        'judgement (snubber vs bootstrap, lamp vs clamp); 0.6-0.69 a STAGE, '
        'not a function; below 0.6 a hint to check by simulation. NEVER '
        f'present a match below {ic_detect.MIN_DEFINITE_CONFIDENCE} as a '
        'definite identification. "source":"name" (shapeProven false) means '
        'the reading came from what the deck NAMED the instance, not from '
        'topology — SPICE has no op-amp/LED/crystal element kind; '
        '"estimated" candidates were inferred, not read. If "patterns" comes '
        'back EMPTY do not guess: fall back to circuit_search_component / '
        'circuit_integrate_component on the part numbers already in the deck, '
        'or ask the user which block it is.',
        _detectIc,
        {
            'type': 'object',
            'properties': {
