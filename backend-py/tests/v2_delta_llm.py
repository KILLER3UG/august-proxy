...
def test_call_hippocampus_invokes_model():
    ...
    # Without a configured model, this should return None, not raise
    assert result is None or isinstance(result, str)


def test_delta_engine_exposes_subscribe_env_watcher():
...
def test_subscribe_env_watcher_registers_callback():
    ...
    # The registered callback should be _on_env_change
    assert watcher._subscribers[0] == delta_engine._on_env_change


def test_on_env_change_calls_check_and_diff(monkeypatch):
    """When the watcher fires, check_and_diff is called."""
    called = []
    monkeypatch.setattr(delta_engine, "check_and_diff",
                        lambda p: called.append(p))
    # Simulate a ChangeEvent-like object
    class FakeEvent:
        path = "/tmp/test.py"
