...
| §12 | Brain popup: image-editor resize (8 handles + drag-anywhere) | ✅ done & verified | | `v4.4.4-popup-image-like-resize` — **drag from anywhere** on the popup (excludes interactive children) + **8 resize handles** (4 corners + 4 edges like Photoshop). "Any drag grows" semantics so accidental shrinking is impossible. **Bug fix:** v4.4.2 tests were passing by accident — `fireEvent.pointerMove(document, {...})` only attaches props to the React synthetic event, but the popup uses native document listeners (it's portaled to body). Tests now dispatch real `MouseEvent`s so coordinates reach native listeners. |
