...
TOOL_DEFINITION = {
    'name': TOOL_NAME,
    'description': 'Spawn multiple sub-agents in parallel to complete independent work items. Each sub-agent receives its own goal and can be given a restricted set of tools. Returns consolidated results when all sub-agents complete.',
    'input_schema': {
        'type': 'object',
        'properties': {
            'workItems': {
...
async def _doSpawn(
    orchestrator: SubagentOrchestrator, session: object, workItems: list[dict[str, Any]], emit: Any | None = None
) -> dict[str, Any]:
