...
- **CamelModel scale-up IN PROGRESS:**
  ...
  - **~37 `BaseModel` subclasses remain across routers** (agents, august, config, cron, desktop_automation, git, manage, mcp, memory, sessions, skills, subagent, terminal, terminal_routes, …). Convert **one router per commit**.
...
- [x] Boundary translation pattern proven (`CamelModel`) — scaling in progress (models + usage done; ~30 routers / ~37 BaseModel classes remain)
...
| CamelModel router scale-up | **In progress** — `models` + `usage` done; ~37 BaseModel classes remain |
...
| Remaining routers with `BaseModel` bodies | ~37 classes | ❌ Next work |

**Suggested next:** `git.py` (`repo_path`), or simple single-word-field routers (`sessions` / `mcp` / `cron`) for mechanical confidence.
...
- **Tests / types (baseline at handoff):** pytest **538** collected/passed · mypy **0 / 176** · ruff clean · CI `type-check.yml`
...
Stop state: user paused after CamelModel **usage** merge. Next action is **choose and convert the next CamelModel router** on a feature branch. Update `docs/REFACTOR_PROGRESS.md` after each merge. Keep Ground Rule 1 — verify this prompt against HEAD before trusting SHAs.
