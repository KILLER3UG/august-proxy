...
def validateToolArguments(toolCall: dict[str, Any], toolDefinitions: list[dict[str, Any]], messages: list[dict[str, Any]] | None=None) -> dict[str, Any]:
    ...
    if messages is None:
        messages = []
    toolName = toolCall.get('function', {}).get('name') or toolCall.get('name')
    if not toolName:
        return {'valid': False, 'error': 'Missing tool name'}
    toolDef = _findToolDefinition(toolName, toolDefinitions)
    if not toolDef:
        return {'valid': True}
    argsRaw = toolCall.get('function', {}).get('arguments', toolCall.get('input', '{}'))
    if isinstance(argsRaw, str):
        ...
        args = argsRaw
    schema = toolDef.get('parameters') or toolDef.get('input_schema') or toolDef.get('function', {}).get('parameters') or {}
    if not schema:
        return {'valid': True}
    args = _applyCompatibilityShims(toolName, args)
    gateResult = _checkProxyExecutionGate(toolName, args, messages)
    if not gateResult['valid']:
        return gateResult
