...
# ── Observability: the black-screen contract ─────────────────────────


@pytest.mark.asyncio
async def test_api_activity_returns_bare_array(client):
...
@pytest.mark.asyncio
async def test_api_requests_has_pending_and_completed(client):
...
async def test_api_stats_has_full_shape(client):
    ...
    body = r.json()
    for k in (
        "totalRequests", "completedRequests", "errorRequests",
        "totalInputTokens", "totalOutputTokens", "totalTokens",
        "estimatedInputCost", "estimatedOutputCost", "estimatedTotalCost",
        "avgDurationMs", "pendingRequests", "mostUsedModel", "mostUsedCount",
        "modelBreakdown", "profileStats",
    ):
        assert k in body, f"missing {k} in stats"


@pytest.mark.asyncio
async def test_api_logs_recent_shape(client):
...
@pytest.mark.asyncio
async def test_api_details_is_array(client):
...
@pytest.mark.asyncio
async def test_api_conversations_is_dict(client):
    _seed_traffic()
    r = await client.get("/api/conversations?period=all")
    assert r.status_code == 200
    body = r.json()
    assert isinstance(body, dict)  # Record<clientType, entries[]>


# ── Brain dashboard: the data-transfer contract (renamed from /ui/*) ─


@pytest.mark.asyncio
async def test_api_brain_status(client, isolated_data):
...
@pytest.mark.asyncio
async def test_api_brain_items(client, isolated_data):
    from app.services import memory_store
...
@pytest.mark.asyncio
async def test_api_brain_vectors(client):
...
@pytest.mark.asyncio
async def test_api_brain_prompt(client):
...
@pytest.mark.asyncio
async def test_api_brain_search(client, isolated_data):
    from app.services import memory_store
...
@pytest.mark.asyncio
async def test_api_brain_learning(client):
...
@pytest.mark.asyncio
async def test_api_brain_guidelines(client, isolated_data):
    from app.services import memory_store
...
@pytest.mark.asyncio
async def test_api_brain_graph(client):
...
@pytest.mark.asyncio
async def test_api_brain_diagnostics(client):
...
@pytest.mark.asyncio
async def test_legacy_ui_routes_are_gone(client):
    ...
    for path in ("/ui/memory/store/status", "/ui/brain/diagnostics"):
        ...
        body = r.text or ""
        # Either the SPA fallback (HTML) or a JSON 404 — but NOT brain JSON.
        is_html = "text/html" in ct or "<!doctype" in body.lower() or "<html" in body.lower()
        is_api_404 = r.status_code == 404 and "application/json" in ct
        assert is_html or is_api_404, (
            f"{path} should be SPA HTML or a JSON 404, got {r.status_code} ({ct})"
        )
        # Crucially, it must not return the brain JSON shape a stale router would.
