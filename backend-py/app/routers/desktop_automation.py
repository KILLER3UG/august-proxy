...
async def health() -> dict[str, object]:
    ...
    if _isAvailable():
        ...
        try:
            ...
            if 'error' in size:
                checks.append({'name': 'screen probe', 'status': 'error', 'message': str(size['error']), 'details': {'solution': 'Run the proxy on a host with an active display.'}})
                overall = 'error'
            else:
                checks.append({'name': 'screen probe', 'status': 'ok', 'message': f"Screen {size['width']}x{size['height']} detected.", 'details': {'solution': None}})
        except Exception as exc:
            checks.append({'name': 'screen probe', 'status': 'error', 'message': f'Screen probe failed: {exc}', 'details': {'solution': 'Ensure a display is attached (not a headless server).'}})
            overall = 'error'
    else:
        overall = 'error'
        checks.append({'name': 'pyautogui', 'status': 'error', 'message': 'pyautogui is not installed.', 'details': {'solution': 'Run `uv sync --extra desktop` to enable desktop automation.'}})
