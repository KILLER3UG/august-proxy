...
def buildTier3(session: dict[str, Any] | None=None) -> str:
    ...
    if failure:
        blocks.append(wrapTag('failure_feedback', failure))
    daemonUpdates = (session or {}).get('subconscious_updates')
    if daemonUpdates:
        blocks.append(wrapTag('subconscious_updates', daemonUpdates))
    ...
    if environment:
        ...
        for e in environment:
            if isinstance(e, dict):
                if 'path' in e:
                    envLines.append(f"File changed: {e['path']} ({e.get('kind', 'modify')}, {e.get('when', 'recently')})")
                if 'git_branch' in e:
...
def buildSystemPrompt(session: dict[str, Any] | None=None, memory: dict[str, Any] | None=None, tools: list[dict[str, Any]] | None=None, agentContext: str | None=None, cachedT12: str | None=None) -> str:
    ...
    if tools:
        coreCount = len(tools)
        merged['tool_guidance'] = f'You have {coreCount} tools available. To learn about any tool, call tool_describe(name). To search for a tool, call tool_search(query, limit).'
    tiers: list[str] = []
    if cachedT12:
