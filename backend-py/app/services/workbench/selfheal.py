...
def buildHints(content: str) -> str:
    ...
    if 'browser' in content.lower() and ('not found' in content.lower() or 'not installed' in content.lower()):
        hints.append('Hint: The browser-use MCP server may not be installed. Try: pip install browser-use playwright && playwright install chromium')
    firstWord = content.strip().split()[0].lower() if content.strip() else ''
    if firstWord in BASH_IN_POWERSHELL_COMMANDS:
        hints.append(f"Hint: '{firstWord}' is a Unix/Linux command that may not work in Windows PowerShell. If running on Windows, try the PowerShell equivalent or check if this environment has Git Bash/WSL.")
    psMatch = POWERSHELL_CMD_PATTERN.search(content)
    if psMatch:
        psCmd = psMatch.group(0)
        hints.append(f"Hint: '{psCmd}' is a PowerShell cmdlet. This environment may be running bash, not PowerShell. Use the Unix/Linux equivalent instead.")
    if 'not recognized' in content.lower():
        hints.append('Hint: This error typically means the command is not available on this system. Check the command spelling or try an alternative approach.')
    if '\\' in content and ('no such file' in content.lower() or 'cannot find' in content.lower()):
        hints.append("Hint: File paths may use forward slashes (/) instead of backslashes (\\). Try using '/' as the path separator.")
    if 'permission denied' in content.lower() or 'access is denied' in content.lower():
        hints.append("Hint: The operation requires elevated permissions. Try using a different location or approach that doesn't require special permissions.")
    if not hints:
        hints.append('Hint: The tool returned an error. Check the parameters and try again. If the issue persists, try a different approach.')
    return '\n\n' + '\n'.join(hints)
