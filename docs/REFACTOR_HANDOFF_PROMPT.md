...
> **Handoff snapshot:** 2026-07-14 · live tracker: `docs/REFACTOR_PROGRESS.md` · tip: verify with `git rev-parse master` (CamelModel git on/after `00904fe`)  
...
**This is a handoff, not a fresh start — and it's already mid-flight.** Phase 0 is signed off. The old multi-branch merge sequence is **finished** (only `master` / `origin/master` remain for active work; a redundant `refactor/b21-app-file-renames` ref may still exist and is safe to delete). Phase 2 CamelModel scale-up is **in progress**: pilot (`models.py`) + `usage.py` + `git.py` are on master. **Pick up from the Progress Log's current step** — do not restart Phase 0 or re-merge closed branches. Before doing anything else, read the Progress Log, then scan the actual repository yourself to confirm what it says is still accurate. Do not take the Progress Log, or any prior audit report it references, at face value — see Ground Rule 1.
...
- **CamelModel scale-up IN PROGRESS:**
  - Pilot: `routers/models.py` (`ModelInfo`, `ModelList`) + `tests/test_camel_model.py`
  - First scale-up: `routers/usage.py` (`UsageRecord`) + `tests/test_camel_model_usage.py` (`40606d5`)
  - Second scale-up: `routers/git.py` (`GitCommand`) + `tests/test_camel_model_git.py` (`00904fe`)
  - **~36 `BaseModel` subclasses remain across routers** (agents, august, config, cron, desktop_automation, manage, mcp, memory, sessions, skills, subagent, terminal, terminal_routes, …). Convert **one router per commit**.
...
- [x] Boundary translation pattern proven (`CamelModel`) — scaling in progress (models + usage + git done; ~36 BaseModel classes remain)
...
**Current step:** Continue **Phase 2 CamelModel scale-up** — next router on a feature branch (suggested: `sessions.py`, `mcp.py`, or `cron.py` for low risk; or `memory` / `terminal` for multi-field value). One router per commit → push → CI → verify → FF-merge → update `docs/REFACTOR_PROGRESS.md`.
...
*Last updated 2026-07-14 (CamelModel git merged). Tip: `master` / `origin/master` (verify `git rev-parse HEAD`; git CamelModel on/after `00904fe`).*
...
| CamelModel router scale-up | **In progress** — `models` + `usage` + `git` done; ~36 BaseModel classes remain |
...
| Remaining routers with `BaseModel` bodies | ~36 classes | ❌ Next work |

**Suggested next:** simple single-word-field routers (`sessions` / `mcp` / `cron`) for mechanical confidence, or `memory` / `terminal` for multi-field value.
...
- **Tests / types (baseline at handoff):** pytest **543** collected · mypy **0 / 176** · ruff clean · CI `type-check.yml`
...
Stop state: CamelModel **git** merge complete (`00904fe`). Next action is **choose and convert the next CamelModel router** on a feature branch. Update `docs/REFACTOR_PROGRESS.md` after each merge. Keep Ground Rule 1 — verify this prompt against HEAD before trusting SHAs.
