...
async def _locatorBbox(page: object, ref: str | None, selector: str | None, text: str | None) -> dict[str, object] | None:
    ...
    try:
        ...
        if not box:
            return None
        return {'x': round(box['x'] + box['width'] / 2), 'y': round(box['y'] + box['height'] / 2), 'width': round(box['width']), 'height': round(box['height'])}
    except Exception:
        return None
...
async def browserOpen(url: str, waitUntil: str='load') -> str:
    ...
    if blocked:
        return _err(blocked)
    validWait = {'load', 'domcontentloaded', 'networkidle', 'commit'}
    if waitUntil not in validWait:
        waitUntil = 'load'
    page, err = await _page()
    if err:
        return err
    try:
        await page.goto(url, wait_until=waitUntil, timeout=_NAVTimeoutMs)
