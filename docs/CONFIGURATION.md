...
| `AUGUST_NGSPICE_EXE` | auto | Explicit ngspice executable for the circuit workbench. The desktop build ships ngspice 45.2 in `src-tauri/resources/ngspice/` and exports this variable at backend spawn; a value you set yourself that points at a real file always wins. Otherwise: bundled copy → PATH → `C:\ngspice\bin` → PATH-style install dirs |
