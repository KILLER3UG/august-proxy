...
_POLLInterval = 5
_RATELimit = 2.0
_IGNOREPatterns = ['*.pyc', '*.pyo', '*.pyd', '*__pycache__*', '*node_modules*', '*.git/objects*', '*.git/index.lock*', '*.swp', '*.swo', '*.DS_Store', '*.log']
...
def watch(workspacePath: str, sessionId: str) -> None:
    ...
    try:
        ...
        watcher.subscribe(lambda e: recordChange(sessionId, {'path': e.path, 'kind': e.kind, 'timestamp': e.timestamp, 'source': e.source}))
...
def _pollingWatch(workspacePath: str, sessionId: str) -> None:
    ...
    try:
        ...
        if branch:
            recordChange(sessionId, {'path': workspacePath, 'kind': 'git', 'timestamp': now, 'source': 'git', 'git_branch': branch})
    except Exception:
        pass
...
class EnvironmentWatcher:
    ...
    def __init__(self, rateLimitSeconds: float=2.0):
        self._rateLimitSeconds = rateLimitSeconds
        self._lastEmit = 0.0
