...
def _resetBus():
    ...
    from app.services import brainEventBus
    brainEventBus.brain_bus._events.clear()
    brainEventBus.brain_bus._subscribers.clear()
    yield

def testEmitAppendsWithIdIsoTimestampAndDefaults():
    """emit() stores {id, category, layer, summary, at, meta}."""
    from app.services.brainEventBus import emitBrainEvent, brainBus
...
def testAddHeuristicPublishesBrainEvent():
    ...
    from app.services.heuristicsService import addHeuristic, removeHeuristic
    rid = addHeuristic(f'v4.3 publish-test {{uuid}}', source='v4_3_test') if False else __import__('uuid').uuid4() and addHeuristic(f"v4.3 publish-test {__import__('uuid').uuid4().hex[:8]}", source='v4_3_test')
...
def testPublisherFailureDoesNotBreakDaemon():
    ...
    from app.services import brainEventBus
    original = brainEventBus.emit_brain_event

    def broken(**__):
        raise RuntimeError('bus exploded')
    brainEventBus.emit_brain_event = broken
    try:
        from app.services.heuristicsService import addHeuristic, removeHeuristic
        ...
        brainEventBus.emit_brain_event = original