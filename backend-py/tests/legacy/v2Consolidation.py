...
def testRunConsolidationAppliesMerges(monkeypatch):
    ...
    async def fakeCall(prompt, **kwargs):
        return json.dumps(
            {
                'merge': [{'keep_id': keepId, 'remove_ids': [removeId], 'merged_rule': 'User prefers Yarn (not NPM)'}],
                'promote': [],
                'delete': [],
            }
        )
...
def testRunConsolidationRecent20Protected(monkeypatch):
    ...
    async def fakeCall(prompt, **kwargs):
        ids = [
            r['id'] for r in conn.execute("SELECT id FROM learned_heuristics WHERE source = 'test-recent'").fetchall()
        ]
...
def testSkillDraftingWritesToStaging(monkeypatch, tmp_path):
    ...
    async def fakeCall(prompt, **kwargs):
        return json.dumps(
            {'name': 'v2-test-skill', 'description': 'A test skill', 'trigger': 'test', 'body': 'Step 1: do it.'}
        )
