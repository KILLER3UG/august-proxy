...
async def getDiskUsage(path: str='.') -> dict[str, object]:
    ...
    try:
        ...
        return {'total': usage.total, 'used': usage.used, 'free': usage.free, 'percent_used': round(usage.used / usage.total * 100, 1) if usage.total else 0}
...
async def listProcesses() -> list[dict[str, object]]:
    ...
    try:
        ...
        try:
            proc = await asyncio.create_subprocess_exec('ps', 'aux', '--sort=-%cpu', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
