...
async def unpin_skill(name: str, request: Request):
    c = _curator(request)
    if not c:
        raise HTTPException(status_code=503, detail="Curator not running")
    if c.unpin(name):
...
async def restore_skill(name: str, request: Request):
    c = _curator(request)
    if not c:
        raise HTTPException(status_code=503, detail="Curator not running")
...
@router.post("/run")
async def run_curation(request: Request, dry_run: bool = False):
    """Run a curation pass now (all transitions, or dry-run)."""
    c = _curator(request)
    if not c:
        raise HTTPException(status_code=503, detail="Curator not running")
    report = c.run_curation(dry_run=dry_run)
