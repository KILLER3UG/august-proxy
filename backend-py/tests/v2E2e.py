...
def testChatWithDaemonBlackboardAndVerifier():
    ...
    session = {'id': sid, 'execution_state': {'phase': 'review', 'step': 3, 'verification_command': 'pytest tests/test_auth.py'}, 'subconscious_updates': [{'name': 'ci_watcher', 'status': 'triggered', 'result': '3 failures in auth.py'}], 'blackboard_state': blackboard_service.read_notes(sid)}
