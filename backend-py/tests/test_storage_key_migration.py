...
Phase 4 schema rename: the table is ``memory_store`` (snake_case). Blob
``key`` values remain camelCase for the public memory API
(``coreMemory`` / ``userProfile``).

These tests build a small file-based SQLite database with the
``memory_store`` schema and exercise ``migrate_storage_keys`` against it.
"""
