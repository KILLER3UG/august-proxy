...
def run_migration(dry_run: bool = False) -> dict[str, Any]:
    ...
    json_path = Path("data/august_learned_guidelines.json")
    stats = {
        "json_found": False,
        "json_count": 0,
        "existing_count": 0,
    ...
    for entry in guidelines:
        ...
        # Check for duplicate by rule text
        dup = conn.execute(
            "SELECT id FROM learned_heuristics WHERE rule = ?", (text,)
        ).fetchone()
        if dup:
            stats["skipped_duplicates"] += 1
            continue
        ...
        conn.execute(
            "INSERT INTO learned_heuristics (rule, source, category, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?)",
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate august_learned_guidelines.json to learned_heuristics table"
    )
