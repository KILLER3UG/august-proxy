...
def testCreatePersistsExtendedSchema(isolatedData):
    a = agentRegistry.createAgent(name='Researcher', description='summarises', role='Researcher', tools=['read_file', 'web_search'], model_alias='fast', actor='test')
...
def testDepthCapBlocks(isolatedData):
    ...
    deep = agentRegistry.createAgent(name='C', parent_id=parent['id'], actor='test')
    for __ in range(4):
        deep = agentRegistry.createAgent(name='D', parent_id=deep['id'], actor='test')
...
def testDeriveChildPermissionsIntersects(isolatedData):
    ...
    child = agentRegistry.createAgent(name='C', parent_id=parent['id'], permissions=['read_file', 'write_file'], actor='test')
